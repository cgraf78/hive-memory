...
use hive_memory::{
    ...
    index, inject, llm, memory, note, outbox, path as memory_path, project, reconcile, search,
    secret, store, visibility, write, write_classify,
};
use serde::Serialize;
...
use cli::{
    context::{ContextArgs, ContextKeyPolicy, ContextSelection},
    curation::{InboxCommand, PromoteArgs},
    eval::EvalCommand,
    projects::ProjectsCommand,
    search::SearchArgs,
    stores::StoresCommand,
    sync::{FlushArgs, OutboxCommand, RefreshArgs, RefreshReport},
...
fn run_hook_session_start(args: HookContextArgs, mut context: CliContext) -> Result<()> {
    ...
    let assembly = cli::context::assemble_cli_context(
        &config,
        &context,
        ContextSelection {
...
fn run_hook_tool_complete(args: HookToolCompleteArgs, context: CliContext) -> Result<()> {
        ...
        && let Some(session_id) = session_id.as_deref()
    {
        ...
        if unrefreshed_receipts > 0 {
            ...
            let mut report = cli::sync::perform(&config, false)?;
...
fn hook_prompt_recall_action(
    ...
    let curated_allowed = cli::search::source_filter_includes_curated(sources);
    ...
    let include_inbox = cli::search::search_include_inbox(false, search_sources);
    ...
    let hits = match cli::search::tantivy_search_if_fresh(config, &store_name, search_input.clone())
    {
        Some(hits) => hits,
...
fn hook_context_key_for_project(
    ...
    Ok(cli::context::context_selection_key(
        &agent_label,
        &[resolved_store.name],
        project_id.as_deref(),
        path_hint,
...
#[cfg(test)]
mod tests {
    use super::{normalize_supersedes, prompt_recall_query, validate_validity_window};
    use crate::cli::context::context_selection_key;
