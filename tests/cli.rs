...
fn hook_session_start_refreshes_local_projection_after_returning() {
    ...
    cargo_bin_cmd!("hm")
        ...
        .env("HIVE_MEMORY_BACKGROUND_REFRESH", "1")
        .args([
            "--config",
            config.to_str().expect("utf8 config"),
            "--as-agent",
...
#[test]
fn retained_last_good_refresh_does_not_acknowledge_new_receipt() {
    ...
    cargo_bin_cmd!("hm")
        ...
        .args([
            ...
            "remember",
            "--text",
            "New memory must remain an unrefreshed receipt.",
        ])
        .assert()
        .success();
    cargo_bin_cmd!("hm")
...
/// Regression for the tantivy cold-cache recall-loss bug: with
/// `search_backend = "tantivy"` and a never-built full-text index, a concurrent
/// rebuild holding the cache-key lock used to make `hm search` create an empty
/// index, skip the (contended) rebuild, and return zero hits — silently
/// stripping results on the primary recall path. The fix makes the
/// never-built/empty + contended-skip case signal a retrieval error so the
/// lexical fallback fires. This holds the rebuild lock externally to force the
/// skip and asserts `hm search` still returns the lexical match.
#[test]
fn search_falls_back_to_lexical_when_tantivy_index_empty_and_lock_held() {
    let dir = temp_dir("search-coldcache-fallback");
    let config = dir.join("config.toml");
    let personal = dir.join("personal");
    let cache = dir.join("cache");
    fs::write(
    ...
    // A store with a memory that the lexical scan can match by exact term.
    cargo_bin_cmd!("hm")
        .args([
            "--config",
            config.to_str().expect("utf8 config"),
            "remember",
            "--text",
            "Chris prefers the zorptastic build profile for releases.",
        ])
        .assert()
        .success();
    ...
    // The fallback must have produced the lexical hit, not an empty result set.
    assert!(
        !stdout.contains("hits: 0"),
        "expected a lexical fallback hit, got empty results.\nstdout: {stdout}\nstderr: {stderr}"
    );
    assert!(
        stdout.contains("zorptastic") || stdout.contains("hits: 1"),
        "expected the matching memory in results.\nstdout: {stdout}"
    );
    // And the degradation must be surfaced, not silent.
    assert!(
